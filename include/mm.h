...
#define EXTRACT_FPN_FROM_PTE(pte) GETVAL(pte,PAGING_PTE_FPN_MASK,PAGING_PTE_FPN_LOBIT )
/*Extract FPN from swapped PTE*/
#define EXTRACT_FPN_SWAPPED_PTE(pte) GETVAL(pte,PAGING_PTE_SWPOFF_MASK,PAGING_PTE_SWPOFF_LOBIT)
...
int __readTLB(struct pcb_t *caller, int vmaid, int rgid, int offset, BYTE *data,int*fpn);
int __writeTLB(struct pcb_t *caller, int vmaid, int rgid, int offset, BYTE value,int*fpn);
...
int print_tlb(struct pcb_t *proc);
