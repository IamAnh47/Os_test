...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    ...
    __swap_cp_page(caller->active_mswp,tagtfpn,caller->mram,vicfpn);
    MEMPHY_put_freefp(caller->active_mswp,tagtfpn);
    //tlb_cache_write(caller->tlb, caller->pid, pgn, tagtfpn,1);
    /* Update page table */
    //pte_set_swap() &mm->pgd;
    ...
    vicpte=0;
    init_pte(&vicpte,1,0,1,0,0,swpfpn);
    ...
    tlb_cache_write(caller->tlb, caller->pid, vicpgn, vicfpn,0);
#endif

    enlist_pgn_node(&caller->mm->fifo_pgn,pgn);
  }

  *fpn = EXTRACT_FPN_FROM_PTE(pte);
 ...
 *
 */
int pg_getval(struct mm_struct *mm, int addr, BYTE *data, struct pcb_t *caller,int *frmn)
{
 ...
 *
 */
int pg_setval(struct mm_struct *mm, int addr, BYTE value, struct pcb_t *caller,int *frmn)
{
  int pgn = PAGING_PGN(addr);
  int off = PAGING_OFFST(addr);
  int fpn;
...
int __read(struct pcb_t *caller, int vmaid, int rgid, int offset, BYTE *data)
{
  ...
  pg_getval(caller->mm, currg->rg_start + offset, data, caller,NULL);
...
int __write(struct pcb_t *caller, int vmaid, int rgid, int offset, BYTE value)
{
  ...
  pg_setval(caller->mm, currg->rg_start + offset, value, caller,NULL);
