...
{                                         // no guarantee all given pages are mapped
  ...
  for(;pgit < pgnum;pgit++)
  {
    ...
    if(fst->in_RAM ==1)
    {
      ...
      pg_getpage(caller->mm,pgn,&fst->fpn,caller);
    }
...
int init_mm(struct mm_struct *mm, struct pcb_t *caller)
{
  ...
  mm->mmap = vma;
  for(int id = 0; id < PAGING_MAX_SYMTBL_SZ;id++){
  mm->allocated[id]=0;
  }
  return 0;
}
...
int print_tlb(struct pcb_t *proc){
  int size = proc->tlb->maxsz;
  struct TLB_entry* arr = proc->tlb->tlb_storage;
  printf("%14s VALID %2s PID %8s page number %2s frame number\n","|","|","|","|");
  for(int i = 0; i<size;i++){
    if(arr[i].pid < 1) continue;
    printf("TLB ENTRY %d %2s %d %6s %d %10s %d %12s %d\n",i,"|",arr[i].valid,"|",arr[i].pid,"|",arr[i].pgnum,"|",arr[i].frmnumb);
  }
  printf("\n");
  return 0;
}
